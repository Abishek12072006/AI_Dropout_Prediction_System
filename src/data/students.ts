export interface Student {
  ...
  orientationAttendance: number; // percentage
...
function calculateRiskScore(s: Omit<Student, 'riskScore' | 'riskCategory' | 'riskFactors' | 'riskExplanation' | 'interventions'>): {
  ...
  // Library usage (max 8 points)
  if (s.libraryUsage < 2) { score += 8; factors.push("Minimal library usage (<2 visits/month)"); }
  else if (s.libraryUsage < 5) { score += 4; factors.push("Low library usage"); }

  // Orientation attendance (max 7 points)
  if (s.orientationAttendance < 40) { score += 7; factors.push("Poor orientation/event attendance (<40%)"); }
  else if (s.orientationAttendance < 60) { score += 4; factors.push("Low orientation attendance"); }
...
function generateInterventions(category: "Low" | "Medium" | "High", factors: string[]): Intervention[] {
  ...
  if (factors.some(f => f.includes("marks") || f.includes("academic") || f.includes("assignment"))) {
    interventions.push({
      id: crypto.randomUUID(), type: "Academic Support",
  ...
  if (category === "High") {
    interventions.push({
      id: crypto.randomUUID(), type: "Mentor Assignment",
...
const rawStudents: Omit<Student, 'riskScore' | 'riskCategory' | 'riskFactors' | 'riskExplanation' | 'interventions'>[] = [
  {
    ...
    commuteDistance: 5, internetAccess: true, caste: "General", libraryUsage: 12, orientationAttendance: 90, chronotype: "Early Bird",
  },
  {
    ...
    commuteDistance: 35, internetAccess: false, caste: "SC", libraryUsage: 1, orientationAttendance: 30, chronotype: "Night Owl",
  },
  {
    ...
    commuteDistance: 20, internetAccess: true, caste: "OBC", libraryUsage: 4, orientationAttendance: 55, chronotype: "Flexible",
  },
  {
    ...
    commuteDistance: 8, internetAccess: true, caste: "General", libraryUsage: 8, orientationAttendance: 85, chronotype: "Early Bird",
  },
  {
    ...
    commuteDistance: 40, internetAccess: false, caste: "OBC", libraryUsage: 1, orientationAttendance: 25, chronotype: "Night Owl",
  },
  {
    ...
    commuteDistance: 12, internetAccess: true, caste: "General", libraryUsage: 6, orientationAttendance: 70, chronotype: "Flexible",
  },
  {
    ...
    commuteDistance: 25, internetAccess: true, caste: "ST", libraryUsage: 2, orientationAttendance: 40, chronotype: "Night Owl",
  },
  {
    ...
    commuteDistance: 10, internetAccess: true, caste: "General", libraryUsage: 10, orientationAttendance: 80, chronotype: "Early Bird",
  },
  {
    ...
    commuteDistance: 45, internetAccess: false, caste: "SC", libraryUsage: 0, orientationAttendance: 15, chronotype: "Night Owl",
  },
  {
    ...
    commuteDistance: 18, internetAccess: true, caste: "OBC", libraryUsage: 5, orientationAttendance: 65, chronotype: "Flexible",
  },
];
